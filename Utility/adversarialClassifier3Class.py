...
flags.DEFINE_string(
    "output_dir", 'output_adv3Class_mimic',
    "The output directory where the model checkpoints will be written.")

flags.DEFINE_integer(
    "total_training_instance", 800,
    "Number of training instance to make for Adversarial Classifier")

## Other parameters
# 80% of i2b2
flags.DEFINE_integer(
    "number_training_instance", 1000,
    "Number of training instance to make for Adversarial Classifier")
...
class ADVProcessor(DataProcessor):
    ...
    def get_i2b2_gen_text(self, data_dir, type='train', is_adv=False, label=1):
        line = []
        main_path = os.path.join(data_dir, 'i2b2_generated')
        # else:
        #     main_path = os.path.join(data_dir, 'generated_124')
        # files = os.listdir(main_path)
        # file = random.choice(files)
        with open(os.path.join(main_path, 'i2b2_gen_data.txt'), 'r') as file:
    ...
    def get_text(self, text_a):
        text_a = text_a.split()
        start_pos = random.randint(0, len(text_a))
    ...
    def get_i2b2_original(self, data_dir, type='train', is_adv=False, label=0):
        line = []
        files = os.listdir(os.path.join(data_dir, 'i2b2_original'))
        ...
        # file = random.choice(files)
        for file in files:
            with open(os.path.join(data_dir, 'i2b2_original', file), 'r') as rfile:
    ...
    def get_mimic_text(self, data_dir, size, type='train'):
        ...
        for i in range(size):
            ...
            # if i > int(size * .75):
            #     label = 0
            if label < 2:  # or random.choice([0, 1, 2]) == 2:
                # if type == 'train':
    ...
    def getLinesGenAll(self, data_dir):
        line = []
        # labels = np.random.randint(2, size=size)
        ...
        # if type == 'train':
        main_path = os.path.join(data_dir, 'generated_774')
        # else:
        #     main_path = os.path.join(data_dir, 'generated_124')
        files = os.listdir(main_path)
        for file in files:
            with open(os.path.join(main_path, file), 'r') as file:
                ...
                for doc in docs:
                    ...
                    if len(text_a) - FLAGS.max_seq_length - 5 > 0:
                        start_pos = random.randint(0, len(text_a) - FLAGS.max_seq_length - 5)
    ...
    def getGenAlli2b2ADV(self, data_dir):
        ...
        # files = os.listdir(main_path)
        # file = random.choice(files)
        with open(os.path.join(main_path, 'i2b2_generated.txt'), 'r') as file:
            docs = file.read().split('=' * 20 + '\n')
            for doc in docs:
                # while len(text_a) - FLAGS.max_seq_length - 5 < 0:
    ...
    def getLines_i2b2(self, data_dir, size, type='train'):
        line = self.get_i2b2_gen_text(data_dir, type, is_adv=False)
        line += self.get_i2b2_original(data_dir, type, is_adv=False)

        merged_lines = []
        while len(merged_lines) < size:
            random.shuffle(line)
            for i in range(0, len(line) - 1, 2):
                merged_lines.append([line[i][0], line[i + 1][0], line[i + 1][2] + line[i][2]])
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        # return self._create_examples(self.getLines_i2b2(data_dir, FLAGS.total_training_instance, type="train"), "train")

        line = self.get_mimic_text(data_dir, int(.8 * FLAGS.number_training_instance), type="train")
        merged_lines = []
        while len(merged_lines) < FLAGS.total_training_instance:
            random.shuffle(line)
            for i in range(0, len(line) - 1):
                merged_lines.append([line[i][0], line[i + 1][0], line[i + 1][2] + line[i][2]])

        return self._create_examples(merged_lines, "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        # return self._create_examples(self.getLines_i2b2(data_dir, int(FLAGS.total_training_instance * .2), type='test'),
        #                              "dev")
        line = self.get_mimic_text(data_dir, int(.2 * FLAGS.number_training_instance), type="dev")
        merged_lines = []
        while len(merged_lines) < int(.2*FLAGS.total_training_instance):
            random.shuffle(line)
            for i in range(0, len(line) - 1, 2):
                merged_lines.append([line[i][0], line[i + 1][0], line[i + 1][2] + line[i][2]])
        return self._create_examples(merged_lines, "dev")

    def get_AdvSuccess_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self.getGenAlli2b2ADV(data_dir), "dev")
