...
def finetune(sess,
             ...
             optimizer='adam', l2_norm_clip=1.0, noise_multiplier=0.001, NB_TRAIN=500,
             overwrite=False):
    ...
    assert model_name not in ['774M',
                              '1558M'] or multi_gpu, "Currently, a modern single GPU cannot finetune the 774M GPT-2 model or larger."
    ...
    if model_name not in ['117M', '124M'] or optimizer == 'dpadam':
    ...
    elif optimizer == 'dpadam':
        ledger = privacy_ledger.PrivacyLedger(
            population_size=NB_TRAIN,
            selection_probability=(batch_size / NB_TRAIN))

        opt = dp_optimizer.DPAdamGaussianOptimizer(
